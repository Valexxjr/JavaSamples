...
public class Index implements Serializable, Closeable {
    ...
    IndexOne2One          productnames;
    IndexOne2N            arrivaldates;

    public void test( Product prod ) throws KeyNotUniqueException {
        assert( prod != null );
        if ( productcodes.contains(prod.productcode)) {
            throw new KeyNotUniqueException( prod.productcode );
        }
        if ( productnames.contains( prod.productname )) {
            throw new KeyNotUniqueException( prod.productname );
        }
    }

    public void put( Product prod, long value ) throws KeyNotUniqueException {
        //test( prod );
        productcodes.put( prod.productcode, value );
        productnames.put( prod.productname, value);
        arrivaldates.put( prod.arrivaldate, Product.productDel, value);
    }

    public Index()  {
        productcodes   = new IndexOne2One();
        arrivaldates = new IndexOne2N();
        productnames 	= new IndexOne2One();
    }
